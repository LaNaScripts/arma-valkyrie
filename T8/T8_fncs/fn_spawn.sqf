...
if (isNil "_MasterArray") exitWith {
	if (T8U_var_DEBUG) then {
		["fn_spawn.sqf", "NO SPAWNING: _masterArray IS NIL!"] spawn T8U_fnc_DebugLog;
	};
false };

if (typeName _MasterArray == "BOOL") exitWith {
	if (T8U_var_DEBUG) then {
		["fn_spawn.sqf", "NO SPAWNING: EVERYBODY WAS ALREADY KILLED", _this] spawn T8U_fnc_DebugLog;
	};
false };

if (typeName _MasterArray != "ARRAY" OR {!( count _MasterArray > 0 )}) exitWith {
	["Something went seriously wrong! Aborting T8U_fnc_Spawn!"] call T8U_fnc_BroadcastHint;
false };
...
// -> ForEach _MasterArray
{
	...
	_posMkr = [_groupArray, 1, "NO-POS-GIVEN", [""]] call BIS_fnc_param;
	...
	if (!( count _vehicleArray > 0) OR {
		_posMkr == "NO-POS-GIVEN"
	} OR {
		_type == "NO-TASK-GIVEN"
	} OR {
		(str (getMarkerPos _posMkr)) == str ([0,0,0])
	}) exitWith {
...
// ------------------ TASK SWITCH --- UNITS WILL BE SPAWNED NOW --------------------------------------------------------------
	...
	switch ( _type ) do {
		...
		default {
			private ["_msg"];
			_msg = format ["The task %1 does not exist!", _type];
	...
	if (! _abort) then {
		if (_groupSide != civilian) then {
			...
			//	Military Units - Add EventHandlers, Routines, Skill
			...
			if (T8U_var_DEBUG) then {
				["fn_spawn.sqf", "UNITS", units _group] spawn T8U_fnc_DebugLog;
			};
			...
			if (T8U_var_DEBUG) then {
				["fn_spawn.sqf", "T8_UnitsVars", [_group getVariable "T8U_gvar_Comm", _group getVariable "T8U_gvar_Origin", _group getVariable "T8U_gvar_Assigned", _group getVariable ["T8U_gvar_Attacked", "not yet"], leader _group getVariable "T8_UnitsVarLeaderGroup"]] spawn T8U_fnc_DebugLog;
			};
	...
	//	Civilian Units
		...
		if (T8U_var_AllowZEUS) then {
...

			{
				...
				if (!isNull _v AND {!(_v in _vehicles)}) then {
...
if (T8U_var_DEBUG_hints) then {
	private ["_msg"];
	_msg = format ["Your Units at %1 are spawned", _posMkrArray];
	[_msg] call T8U_fnc_BroadcastHint;
};
